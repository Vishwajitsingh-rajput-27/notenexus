...
router.post('/test-send', auth, async (req, res) => {
  try {
    ...
    log.info('Test reminder dispatch', { to: req.user.email });
    ...
    log.error('Test reminder failed', err);
    res.status(500).json({
      success: false,
      error: err.message,
  ...
  try {
    ...
    switch (scheduleType) {
      case 'today':
        ...
        log.info('Reminder scheduled for today', { at: nextReminder.toISOString() });
        break;
      ...
      case 'custom_date':
        ...
        nextReminder = oneShotAt;
        log.info('Reminder scheduled for custom date', { at: nextReminder.toISOString() });
        break;

      case 'interval_minutes':
        finalIntervalMins = Math.max(1, parseInt(intervalMinutes) || 60);
        nextReminder = new Date(Date.now() + finalIntervalMins * 60 * 1000);
        log.info('Reminder scheduled on minute interval', { intervalMins: finalIntervalMins, firstAt: nextReminder.toISOString() });
        break;
      ...
      default:
        ...
        log.info('Repeating reminder — first send NOW', { intervalDays: finalIntervalDays });
        break;
    }
    ...
    if (sendWhatsApp && !resolvedPhone) {
      ...
      if (session) {
        ...
        log.info('WhatsApp delivery via linked phone', { phone: resolvedPhone });
      } else {
        log.warn('WhatsApp enabled but no linked session found');
      }
    }
    ...
    log.ok('Reminder created', { id: reminder._id });
    ...
    if (scheduleType === 'repeating' || scheduleType === 'interval_minutes' || !scheduleType) {
      try {
        const { sendReminder } = require('../services/reminderService');
        log.info('Sending first reminder immediately');
        ...
        await reminder.save();
        log.ok('First reminder sent', { nextAt: reminder.nextReminder.toISOString() });

      } catch (err) {
        log.error('First reminder send failed', err);
    ...
    log.error('POST /reminders failed', err);
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    log.error('GET /reminders failed', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    log.info('Reminder cancelled', { id: req.params.id });
    res.json({ success: true, message: 'Reminder cancelled successfully' });
  } catch (err) {
    log.error('DELETE /reminders failed', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/whatsapp-phone', auth, async (req, res) => {
  try {
    ...
    log.error('GET whatsapp-phone failed', err);
    res.status(500).json({ error: err.message });
  }
});
